<?php 
    $theo_nganh_nghe_ut = $action_work->theo_nganh_nghe_ut($trang, 10, $_GET['search']);
    $theo_nganh_nghe_kut = $action_work->theo_nganh_nghe_kut($trang, 30, $_GET['search']);
?>
<div class="gb-page-sanpham_ruouvang">
    ...
    <div class="container">
        <div class="row">
            <div class="col-md-9">
                <?php 
                foreach ($theo_nganh_nghe_ut['data'] as $item) { 
                ...
                <?php } ?>
                <div>
                    <?= $theo_nganh_nghe_ut['paging'] ?>
                </div>
                <hr style="border: 1px solid red;">
                <?php 
                foreach ($theo_nganh_nghe_kut['data'] as $item) { 
                    $dodamdo = $action_work->check_damdo($item['don_tuyen_id']);//var_dump($item);
                    $moi = $action_work->don_moi($item['don_tuyen_id']);
                ?>
                <div class="gb-product_ruouvang-item">
