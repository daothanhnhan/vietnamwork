  ...
  $city = $action->getList('city', '', '', 'id', 'asc', '', '', '');
    ...
    function don_tuyen () {
      ...
      if (isset($_POST['don_tuyen'])) {
        ...
        $sql = "INSERT INTO don_tuyen (name, alias, cap_bac, career, city, salary1, salary2, mota, yeucau, email, company, quymo, address, phucloi1, phucloi2, phucloi3, image, nha_tuyen_dung_id, ngay, it, district, street, ky_nang, company_note, deadline, views) VALUES ('$name', '$alias', $cap_bac, '$career', '$city', $salary1, $salary2, '$mota', '$yeucau', '$email', '$company', '$quymo', '$address', '$phucloi1', '$phucloi2', '$phucloi3', '$image', $nha_tuyen_dung_id, '$ngay', $it, $district, $street, '$ky_nang', '$company_note', '$deadline', 0)";//echo $sql;
        $result = mysqli_query($conn_vn, $sql);
        if ($result) {
          $_SESSION['don_tuyen_gbvn'] = mysqli_insert_id($conn_vn);
          echo '<script type="text/javascript">alert(\'Bạn đã tạo đơn tuyển dụng thành công!\');window.location.href="/phi-tuyen-dung";</script>';
        } else {
    ...
    don_tuyen();
    // var_dump($_SESSION);
    $address_arr = array($nha_tuyen_dung['address']);
    $quan_ntd = $action_work->get_quan($address_arr);
    $quan_count = count($quan_ntd);
    $duong_ntd = $action_work->get_duong($quan_ntd[0]['id']);
  ...
  <div class="container">
    ...
    <form class="form-horizontal" action="" method="post" enctype="multipart/form-data">
      <div class="form-group">
        ...
        <div class="col-sm-10">
          <input type="text" class="form-control" id="name" placeholder="" name="name" onchange="ChangeToSlug()" required>
      ...
      <div class="form-group">
        ...
        <div class="col-sm-10">
          <div class="checkbox">
            <label><input type="checkbox" value="1" name="it" style="width: 50px;"></label>
          </div>
        </div>
      </div>
      <div class="form-group">
        ...
        <div class="col-sm-10">
          <select class="form-control chosen-select" id="city" name="city[]" onchange="cityf(this)" data-placeholder="Yêu cầu chọn nơi làm việc" multiple required>
            ...
            <option value="<?= $item['id'] ?>" <?= $item['id']==$nha_tuyen_dung['address'] ? 'selected' : '' ?> ><?= $item['name'] ?></option>
            <?php } ?>
          </select>
        </div>
      </div>
      ...
      <div class="form-group">
        ...
        <div class="col-sm-5">          
          <input type="text" class="form-control " id="salary1" placeholder="Từ" name="salary1" onkeyup="money(this)" required>
        </div>
        <div class="col-sm-5">          
          <input type="text" class="form-control " id="salary2" placeholder="Đến" name="salary2" onkeyup="money(this)" required>
        </div>
      </div>
      <div class="form-group">
        <label class="control-label col-sm-2" for="ky_nang">Kỹ năng:</label>
        <div class="col-sm-10">
          <input type="text" class="form-control" id="ky_nang" placeholder="" name="ky_nang" required>
        </div>
      </div>
      <div class="form-group">
      ...
      <div class="form-group">
        ...
        <div class="col-sm-10">
          <textarea class="form-control" rows="10" id="mota" name="mota" required></textarea>
        </div>
      </div>
      <div class="form-group">
        <label class="control-label col-sm-2" for="yeucau">Yêu cầu công việc:</label>
        <div class="col-sm-10">
          <textarea class="form-control" rows="10" id="yeucau" name="yeucau" required></textarea>
        </div>
      </div>
      <div class="form-group">
        <label class="control-label col-sm-2" for="email">Địa chỉ email nhận hồ sơ:</label>
        <div class="col-sm-10">
          <input type="text" class="form-control" id="email" placeholder="" name="email" value="<?= $nha_tuyen_dung['email'] ?>" required>
        </div>
      </div>
      <h2>Công ty</h2>
      <div class="form-group">
        ...
        <div class="col-sm-10">
          <input type="text" class="form-control" id="company" placeholder="" name="company" value="<?= $nha_tuyen_dung['company'] ?>" required>
        </div>
      </div>
      <div class="form-group">
        <label class="control-label col-sm-2" for="quymo">Quy mô công ty:</label>
        <div class="col-sm-10">
          <input type="text" class="form-control" id="quymo" placeholder="" name="quymo" >
        </div>
      </div>
      <div class="form-group">
        <label class="control-label col-sm-2" for="company_note">Mô tả công ty:</label>
        <div class="col-sm-10">
          <textarea class="form-control" rows="10" id="company_note" name="company_note" ></textarea>
        </div>
      </div>
      <div class="form-group">
        <label class="control-label col-sm-2" for="address">Địa chỉ công ty:</label>
        <div class="col-sm-10">
          <input type="text" class="form-control" id="address" placeholder="Không bắt buộc" name="address" >
        </div>
      </div>
      <div class="form-group">
        <label class="control-label col-sm-2" for="phucloi1">Phúc lợi từ công ty:</label>
        <div class="col-sm-10">
          <input type="text" class="form-control" id="phucloi1" placeholder="Mức lương cạnh tranh" name="phucloi1" >
          <input type="text" class="form-control" id="phucloi2" placeholder="Các chế độ ưu đãi đặc biệt từ mobifone" name="phucloi2" >
          <input type="text" class="form-control" id="phucloi3" placeholder="Cơ hội thăng tiến" name="phucloi3" >
        </div>
      </div>
      <div class="form-group">
      ...
      <div class="form-group">        
        <div class="col-sm-offset-2 col-sm-10">
          <button type="submit" class="btn btn-default" name="don_tuyen">Tạo đơn tuyển</button>
        </div>
      </div>
    </form>
  </div>
