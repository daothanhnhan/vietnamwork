<?php 
  ...
  function noi_bat () {
    ...
    if (isset($_POST['noibat'])) {
      ...
      $sql = "INSERT INTO nha_tuyen_hot (nha_tuyen_dung_id, name, tip, note, banner, logo, state) VALUES ($nha_tuyen_dung_id, '$name', '$tip', '$note', '$banner', '$logo', 0)";
      // echo $sql;
      $result = mysqli_query($conn_vn, $sql);
      if ($result) {
        echo '<script type="text/javascript">alert(\'Bạn đã thêm mới nhà tuyển dụng nổi bật thành công.\');window.location.href="/sua-nha-tuyen-noi-bat"</script>';
      } else {
...
<div class="container">
  <h2>Thêm nhà tuyển dụng nổi bật</h2>
  <form class="form-horizontal" action="" method="post" enctype="multipart/form-data">
    <div class="form-group">
      <label class="control-label col-sm-2" for="name">Tiêu đề:</label>
      <div class="col-sm-10">
        <input type="text" class="form-control" id="name" placeholder="Nhập tiêu đề" name="name" required>
      </div>
    </div>
    <div class="form-group">
      <label class="control-label col-sm-2" for="tip">Chú thích:</label>
      <div class="col-sm-10">          
        <input type="text" class="form-control" id="tip" placeholder="Nhập chú thích" name="tip" required>
      </div>
    </div>
    <div class="form-group">
    ...
    <div class="form-group">
      ...
      <div class="col-sm-10">          
        <input type="file" class="form-control" id="banner" placeholder="" name="banner" required>
      </div>
    </div>
    <div class="form-group">
    ...
    <div class="form-group">        
      <div class="col-sm-offset-2 col-sm-10">
        <button type="submit" name="noibat" class="btn btn-default">Thêm</button>
      </div>
    </div>
  </form>
</div>