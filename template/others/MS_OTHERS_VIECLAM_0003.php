<?php 
    ...
    $info = $action->getDetail('ung_vien', 'id', $_SESSION['user_id_uv_gbvn']);//var_dump($info);
    if ($info['quoc_gia']==1) {
        $city_info = $city;
    } else {
        $city_info = array();
    ...
    $ky_nang = $action->getList('ky_nang', 'ung_vien_id', $_SESSION['user_id_uv_gbvn'], 'id', 'asc', '', '', '');
    $ky_nang_count = count($ky_nang);

    $ngon_ngu = $action->getList('ngon_ngu', 'ung_vien_id', $_SESSION['user_id_uv_gbvn'], 'id', 'asc', '', '', '');
    $ngon_ngu_count = count($ngon_ngu);

    $kinh_nghiem = $action->getList('kinh_nghiem', 'ung_vien_id', $_SESSION['user_id_uv_gbvn'], 'id', 'asc', '', '', '');
    $kinh_nghiem_count = count($kinh_nghiem);

    $hoc_van = $action->getList('hoc_van', 'ung_vien_id', $_SESSION['user_id_uv_gbvn'], 'id', 'asc', '', '', '');

    $tham_khao = $action->getList('tham_khao', 'ung_vien_id', $_SESSION['user_id_uv_gbvn'], 'id', 'asc', '', '', '');
?>
...
<div class="gb-taohoso-vnwork">
    <div class="container">
        ...
        <div class="gb-taohoso-vnwork-info-cv">
            <button type="" name="add_hs">Tạo hồ sơ</button>
        </div>
</form>
    </div>
</div>
