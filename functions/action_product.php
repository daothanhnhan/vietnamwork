...
/*----- liệt kê danh sách product đa cấp dựa vào Product Cat Id cao nhất -------*/
public function getProductList_byMultiLevel_orderProductId_filter($idProductCat,$order_productId,$trang, $limit, $page){
    ...
    if ($idProductCat != "" && $nameColIdProductCat != "") {
        ...
        $product_brand = $_GET['hang'];
        $product_sex = $_GET['gioitinh'];
        $product_price = $_GET['gia'];
        ...
        if ($product_sex != "") {
            $where .= " AND product_sex = $product_sex";
        }

        if ($product_size != "") {
            $where .= " AND product_size_arr LIKE '%$product_size%'";
        }

        if ($product_price != "") {
            if ($product_price == 1) {
                $where .= " AND product_price between 0 and 1000000";
            } elseif ($product_price == 2) {
                $where .= " AND product_price between 1000000 and 1500000";
            } elseif ($product_price == 3) {
                $where .= " AND product_price > 1500000";
            }
        }
    ...
    if ($trang != "" && $limit != "") {
        ...
        $config = array(
            ...
            'link_full'=> "/index.php?page=$page&hang=$product_brand&gioitinh=$product_sex&size=$product_size&gia=$product_price&trang={trang}",

            'link_first'=> "/index.php?page=$page&hang=$product_brand&gioitinh=$product_sex&size=$product_size&gia=$product_price",
...
/*----- liệt kê danh sách product đa cấp dựa vào Product Cat Id cao nhất -------*/
public function getProductList_byMultiLevel_orderProductId_Sale($idProductCat,$order_productId,$trang, $limit, $page){
    ...
    if ($idProductCat != "" && $nameColIdProductCat != "") {
        ...
        $cond = "";
        $where = "WHERE product_price_sale != 0";
    }
...
public function getCountProduct ($id) {
    global $conn_vn;
    $sql = "SELECT * FROM product WHERE productcat_ar LIKE '%$id%'";
    $result = mysqli_query($conn_vn, $sql);
    $count = mysqli_num_rows($result);
    return $count;
}

public function getCountProduct1 ($id) {
    global $conn_vn;
    $sql = "SELECT * FROM product WHERE productcat_ar LIKE '%$id%'";
    $result = mysqli_query($conn_vn, $sql);
    $count = mysqli_num_rows($result);
    ...
    if ($num > 0) {
        while ($row = mysqli_fetch_assoc($result)) {
            $id1 = $row['productcat_id'];
            $sql1 = "SELECT * FROM product WHERE productcat_ar LIKE '%$id1%'";
