...
class action_menu extends library{
    ...
    /*-----------------------------*/
    public function showMenu_byMultiLevel_mainMenuCarrot($list_menu,$valParent_menu = 0,$lang, $level = 0){
        $cate_child = array();
        foreach ($list_menu as $key => $item)
        {
            // Nếu là chuyên mục con thì hiển thị
        ...
        if ($cate_child) {
            ...
            foreach ($cate_child as $key => $item) {
                ...
                if ($level == 0) {
                   echo '<a href="'.$url.'" class="link_item_main_menu">'.$row[$this->nameColTitle_menuLanguage].'</a>';
                } else {
                    echo '<a href="'.$url.'" class="link_item_sub_menu">'.$row[$this->nameColTitle_menuLanguage].'</a>';
                }
                 
                    $this->showMenu_byMultiLevel_mainMenuCarrot($list_menu,$item[$this->nameColID_menu],$lang, $level+1);                
                echo '</li>';            
            }
            echo '</ul>';        
        } 
    ...
    public function subMenu ($list_menu,$valParent_menu = 0,$lang, $level = 0) {
        $cate_child = array();
        foreach ($list_menu as $key => $item)
        {
            // Nếu là chuyên mục con thì hiển thị
        ...
        if ($cate_child) {
            return 'gb-daugiabtn-menu-huongdan-a';
        } else {
            return '';
        }
    }
